...
// 				</div>
// 			</a>
// 			<nav className="header hidden bg-white text-md sm:block">
// 				<ul
...
// 						<a
// 							className="block whitespace-nowrap px-2 py-2 text-md text-slate-600 no-underline"
// 							href="/"
// 						>
// 							Home
// 							<span></span>
...
function Header() {
	...
	const [isMenuOpen, setMenuOpen] = useState(false);
	return (
		<header className="header mx-auto flex w-screen items-center justify-between py-4 px-6 top-0 z-10 fixed bg-white">
			<a href="#">
				<div className="w-full text-center text-lg font-extrabold sm:w-fit sm:text-left">
					<span className="text-cyan-500">J</span>
					<span className="text-slate-800">IVE</span>
			...
			<nav
				className={`
				bg-white text-base opacity-50
				transition transform -right-1/2 fixed top-0 z-20 h-full w-1/2 overflow-y-auto py-4 sm:hidden -translate-x-full shadow-2xl
			${isMenuOpen ? "" : "-right-full transform"}`}
			>
				<ul className={`header-ul-mobile ${isMenuOpen ? "open" : ""}`}>
					...
					<li className="text-right">
						...
						whitespace-nowrap px-2 py-2 text-sm text-slate-600 no-underline transition hover:text-slate-900"
							href="https://scholar
						.google.com/citations?view_op=list_works&hl=en&hl=en&user=WMYcG5EAAAAJ&sortby=pubdate"
							target="_blank"
							rel="noopener noreferrer"
						>
							Google Scholar
